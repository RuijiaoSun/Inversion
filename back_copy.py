...
def forward(m, M):
    ...
    # depths = [-100, -50, 0, 50]  # specify the position (first boundary) of each layer (first boundary is where the field is defined)
    z_pos = [-100, 50]
    ...
    D = [z_pos[0], z_pos[1], 0, 0.5*(z_pos[1]-z_pos[0])]
    x = np.linspace(D[2], D[3], M)
...
m0 = np.array([1, 1.4, 1, 1], dtype='float')
E = forward(m0, M)
...
def jacobian(m_cur):
    ...
    for j in range(N):
        ...
        for i in range(M):
            J_cur[i][j] = (F_nxt[i] - F_cur[i]) / (m_nxt[j] - m_cur[j]) / F_nxt_mean
    return J_cur, W
...
while rms > 0.1:
    ...
    for u_i in np.linspace(0, U, 10):
        m_cur_i = np.dot(np.linalg.inv(u_i * np.dot(alpha.T, alpha) + np.dot(np.dot(W, J_cur).T, np.dot(W, J_cur))),
                         np.dot(np.dot(W, J_cur).T, np.dot(W, d_cur)))
        # m_cur_i = np.dot(np.linalg.inv(u_i * np.dot(alpha.T, alpha) + np.dot(np.dot(W, J_cur).T, np.dot(W, J_cur))), np.dot(np.dot(W, J_cur).T, np.dot(W, d_cur)))
